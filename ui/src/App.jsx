...
function App() {
  ...
  useEffect(() => {
    ...
    axios.get(`${API_BASE}/problems`)
      ...
      .catch(err => setError('Failed to fetch problems from backend. Is it running?'));
  }, []);
  ...
  return (
    <div className="app-container">
      <div className="sidebar glass">
        ...
        {error && <div className="text-secondary" style={{ color: 'var(--danger-color)' }}>{error}</div>}
      ...
      <div className="main-content">
        <div className="visualizer-header">
          ...
          {activeProblem && <p className="text-secondary" style={{ margin: '8px 0 0 0' }}>{activeProblem.description}</p>}
        </div>

        {activeProblem && activeProblem.inputs && (
          <div className="custom-inputs-section glass">
            <h3 style={{ marginTop: 0, marginBottom: '12px' }}>Test Case Inputs</h3>
            <div className="inputs-grid">
              {activeProblem.inputs.map(inp => (
                <div key={inp.name} className="input-group">
                  <label>{inp.name} <span className="type-badge" style={{ opacity: 0.6, fontSize: '0.8em', marginLeft: '4px' }}>{inp.type}</span></label>
                  <input 
                    type="text" 
                    value={customInputs[inp.name] || ''} 
                    onChange={(e) => handleInputChange(inp.name, e.target.value)} 
                    style={{ width: '100%', padding: '8px', background: 'rgba(255,255,255,0.05)', border: '1px solid var(--border-color)', color: 'inherit', borderRadius: '4px', marginTop: '4px' }}
                  />
                </div>
              ))}
            </div>
            <button className="primary" onClick={handleRunCustom} disabled={loading} style={{ marginTop: '16px' }}>Run with Custom Inputs</button>
          </div>
        )}
        
        {loading ? (
          <div className="visualizer-container glass" style={{ alignItems: 'center', justifyContent: 'center' }}>
            <div className="text-secondary" style={{ fontSize: '1.2rem', animation: 'bounce 1s infinite' }}>Running Algorithm...</div>
          </div>
        ) : (
